...
const PORTABILITY_SUBSET_EXT_NAME: &str = "VK_KHR_portability_subset\0";

pub fn required_device_extension_names() -> Vec<*const i8> {
    vec![PORTABILITY_SUBSET_EXT_NAME.as_ptr() as *const i8]
}

#[cfg(target_os = "macos")]
pub fn required_instance_extension_names() -> Vec<&'static str> {
    vec![
        Surface::name().to_str().unwrap(),
        MacOSSurface::name().to_str().unwrap(),
        DebugUtils::name().to_str().unwrap(),
        GetPhysicalDeviceProperties2::name().to_str().unwrap(),
    ]
}
